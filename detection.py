...
def process(source):
    ...
    helmet_riders_plates = ['ARC-123',]
    non_helmet_riders_plates = ['RNX-1138','KGD-3079',]
    saved_helmet_riders = set()
    saved_non_helmet_riders = set()
    while(cap.isOpened()):
        ret, frame = cap.read()
                ...
                for hd in head_list:
                    ...
                    if inside_box([x1r, y1r, x2r, y2r], [x1h, y1h, x2h, y2h]): 
                        ...
                        if helmet_present[0] == True:
                            ...
                            for plate in helmet_riders_plates:
                                if plate not in saved_helmet_riders:

                                    try:
                                        insert_query = "INSERT INTO LicensePlate (HelmetRiders) VALUES (%s)"
                                        cursor.execute(insert_query, (plate,))
                                        conn.commit()
                                        saved_helmet_riders.add(plate)
                                ...
                                for num in number_list:
                                    ...
                                    if inside_box([x1r, y1r, x2r, y2r], [x1_num, y1_num, x2_num, y2_num]):
                                        ...
                                        try:
                                            ...
                                            for i in result:  
                                                print(i[2][1][0],"_______________________line______________")
                                                HelmetRiders=i[2][1][0]
                            ...
                            frame = cv2.putText(frame, f'{round(helmet_present[1], 1)}', (x1h, y1h + 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)

                           
                            for plate in non_helmet_riders_plates:
                                if plate not in saved_non_helmet_riders:
                              
                                    try:
                                        insert_query = "INSERT INTO LicensePlate (Non_HelmetRiders) VALUES (%s)"
                                        cursor.execute(insert_query, (plate,))
                                        conn.commit()
                                        saved_non_helmet_riders.add(plate)
                                    except Exception as e:
                                        print(e)
                                        print('could not save number plate')
                                ...
                                for num in number_list:
                                    ...
                                    if inside_box([x1r, y1r, x2r, y2r], [x1_num, y1_num, x2_num, y2_num]):
                                        ...
                                        try:
                                            ...
                                            result2 = ocr.ocr(img_path, cls=True)
                                            print(result2,"-----------result22222222222---------------")

                                            for i in result2:
                                                    print(i,"---------------line222-------------------")
                                                    nested_element = i[0]
                                                    number_plate = nested_element[1][0]
                                                    print(number_plate)

                                                    Non_HelmetRiders=number_plate
